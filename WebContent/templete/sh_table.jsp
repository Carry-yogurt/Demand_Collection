						...
						<div class="container-fluid">
							<div class="row">
								...
								<div class="col-md-2" style="text-align: center;">
									<button type="button" onclick="shtg(2)" class="btn btn-success">审核通过</button>
								</div>
								<!-- <div class="col-md-1" style="text-align: center;"><label for="glcs">请选择分配的处室</label></div> -->
								<div class="col-md-4" style="text-align: center;">

									<select required="required" name="glcs" id="glcs"
										class="form-control">
										<option value="">-------------------------请选择管理处室-------------------------</option>
										<c:forEach var="item" items="${glcs_list}">
											<option value="${item.gid}">${item.gid}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${item.gname}</option>
										</c:forEach>

									</select>
								</div>
								<div class="col-md-2" style="text-align: center;">
									<button type="button" onclick="shtg(3)" class="btn btn-danger">审核不通过</button>
								</div>
								<div class="col-md-4" style="text-align: center;">
									<input required="required" name="xsshyj" id="xsshyj"
										class="form-control" placeholder="如果不通过，请填写审核意见">
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	function shtg(type){
		var n=type;
		var glcs=document.getElementById("glcs");
		var xsshyj=document.getElementById("xsshyj");
		var xqid=${xb.xqid};
		//document.getElementById("shuaxin")
		//alert(glcs.value);
		if(type=="2"){
			//alert(type+"2");
			if(glcs.value==""||glcs.value=="0"){
				alert("请选择相应的管理处室");
				glcs.focus();
				return false;
			}
			var url = "xQSH_Servlet?n="+n+"&xqid="+xqid+"&glcs="+glcs.value;
			...
			var url = "xQSH_Servlet?n="+n+"&xqid="+xqid+"&xsshyj="+xsshyj.value;
		}
		alert(url);
	...
	$(function(){
		alert("${xb.kjhdlx}");
		if("${xb.kjhdlx}"=="基础研究"){
			$("#xkfl_tr").css('display','table-row');
		}else{
			$("#xqjsyyly_tr").css('display','table-row');
			$("#xqjsyyhy_tr").css('display','table-row');
		}
	});
	</script>
</html>
